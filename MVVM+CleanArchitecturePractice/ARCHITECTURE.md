...
- 破線枠のボックスが **プロトコル（抽象）**、実線枠のボックスが **具象** です。
- 矢印はすべて「依存」を表し、矢印の先が依存される側（＝抽象）です。**実装（準拠）もそのプロトコルへの依存の一種**なので、線種を分けていません。
- 各プロトコルには「**使う側（上）**」と「**実装する側（下）**」の両方から依存が向きます。この「依存が抽象へ集約する」構図こそが依存性逆転（DIP）です。
